        ...
        with st.sidebar:
            ...
            if st.button("Add Class"):
                if class_input.strip():
                    exists = supabase.table("classroom_settings").select(
                        "*").eq("class_name", class_input).execute().data
                    if exists:
                ...
                if delete_target.strip():
                    ...
                    if st.text_input("Type DELETE to confirm") == "DELETE":
                        supabase.table("attendance").delete().eq(
                            "class_name", delete_target).execute()
...
def class_controls(supabase):
    try:
        classes_resp = supabase.table
        ("classroom_settings").select("*").execute()
    ...
    other_open = [c["class_name"] for c in classes if c.get(
        "is_open") and c["class_name"] != selected_class]
            ...
            if other_open:
                ...
                try:
                    supabase.table("classroom_settings").update(
                        {"is_open": True}).eq("class_name", selected_class).execute()
                    st.rerun()
                except Exception:
    ...
    with col2:
        if st.button("Close Attendance"):
            try:
                supabase.table("classroom_settings").update(
                    {"is_open": False}).eq("class_name", selected_class).execute()
                st.rerun()
            except Exception:
                logger.exception("Failed to close attendance")
                st.error("Failed to close attendance")

    with st.expander("Update code and limit"):
        new_code = st.text_input("New Code", value=config["code"])
        new_limit = st.number_input(
            "New Limit", max_value=1, value=config["daily_limit"], step=1)
        if st.button("Save Settings"):
            try:
                supabase.table("classroom_settings").update(
                    {"code": new_code, "daily_limit": new_limit}).eq("class_name", selected_class).execute()
                st.success("Setting Updated")
                st.rerun()
            except Exception:
                logger.exception("Failed to updated settings")
...
def show_matrix_and_push(supabase, repo, selected_class):
    try:
        records_resp = supabase.table("attendance").select(
            "*").eq("class_name", selected_class).order("date", desc=True).execute()
    ...
    if records:
        ...
        df["status"] = "P"
        pivot_df = df.pivot_table(index=["roll_number", "name"], columns="data",
                                  values="status", aggfunc="first", fill_value="A").reset_index()
        ...
        st.download_button("Download CSV", pivot_df.to_csv(
            index=False).encode(), f'{selected_class}_matrix.csv', "text/csv")

        ######################################################

        if st.button("Push to Github"):
            if repo is None:
                st.error("Github not configured. Cannot pus file.")
                return
