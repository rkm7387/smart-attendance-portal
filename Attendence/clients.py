...
def create_github_repo():
    """Create and return a (Github, repo) tuple. if Github settings are not provided, returns (None,None)"""
    try:
        token = get_env("GITHUB_TOKEN")
        username = get_env("GITHUB_USERNAME")
        repo_name = get_env("GITHUB_REPO")

        if not token or not username or repo_name:
            logger.info(
                "GitHub credentials not fully configured; GitHub features will be disabled.")
            return None, None

        gh = Github(token)
        repo = gh.get_user(username).get_repo(name=repo_name)
        return gh, repo
    except Exception:
